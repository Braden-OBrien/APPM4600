...
methods = [eval_gauss_quad, eval_composite_trap, eval_composite_simpsons]
...
for method in methods:
  ...
  ax[1].legend()
  plt.savefig('figerr_'+str(method))

  # plot the adaptive mesh for M=2
  fig,ax = plt.subplots(1,1)
  ax.semilogy(mesh,f(mesh),'ro',label=labl)
  ax.legend()
  plt.savefig('figplot_'+str(method))
  
  print('adaptive quad error for', str(method), 'is', err_new)
