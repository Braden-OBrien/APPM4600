...
def driver():
    ...
    print('we returning out here')
    ################################################################################
    # plot of the trajectory of steepest descent against contour map
    nX=400;nY=400;nZ=400;
    (X,Y,Z) = np.meshgrid(np.linspace(-2,10,nX),np.linspace(-2,8,nY),np.linspace(-2,10,nZ));
